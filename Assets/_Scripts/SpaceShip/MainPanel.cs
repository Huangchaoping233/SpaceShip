...
namespace SpaceShip{
	...
	public class MainPanel : BaseUIForm {
		...
		void Awake(){
			...
			ReceiveMessage ("MainPanelCount", delegate(KeyValuesUpdate kv) {
				int type = (int)kv.Values;
				scroll.ShowItem (type);
			});


			ReceiveMessage ("MainPanelActive",delegate(KeyValuesUpdate kv) {
				bool active = ( bool)kv.Values;
				SetHideOrShow(active);
			});

			ReceiveMessage ("MpSetPlayGridItem",delegate(KeyValuesUpdate kv) {
			...
			ReceiveMessage ("MpPlayerSaveShip",delegate(KeyValuesUpdate kv) {
//				string path = (string)kv.Values;
				shipCtr.SaveData();
